...
public class TaskManagerView extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (task.getTaskid() == 0 || task.isClozeTask()) {
			...
			out.println(" <a href=\"#\" onclick=\"toggleVisibility('clozehelp'); return false;\">(?)</a><br><span style=\"display:none;\" id=clozehelp><b>Hilfe:</b><br>Jede Lücke besteht aus drei zwei Teilen, die in geschweifte Klammern eingefasst und durch Doppelpunkt getrennt sind:<ol><li><b>Teil Kennzeichnung der Art der Lücke. Es gibt folgende Typen von Lücken:</b><ul><li>Kurzantworten (SHORTANSWER), Groß-/Kleinschreibung wird berücksichtigt.</li><li>Kurzantworten (SHORTANSWER_NC), Groß-/Kleinschreibung ist unwichtig.</li><li>Kurzantworten (SHORTANSWER_IS), Leerzeichen bei studentischen Eingaben werden vor dem Vergleich entfernt.</li><li>Kurzantworten (SHORTANSWER_NC_IS), Leerzeichen bei studentischen Eingaben werden vor dem Vergleich entfernt, Groß-/Kleinschreibung ist unwichtig.</li><li>Multiple-Choice-Antwort (MULTICHOICE), als Dropdown-Auswahlmenü im Lückentext. Die Reihenfolge der Antwortoptionen wird automatisch lexikographisch sortiert.</li><li>Numerische Antwort (NUMERICAL).</li></ul>Es muss exakt die obige Schreibweise (Großbuchstaben!) verwendet werden. Hinter dem Typ steht immer ein Doppelpunkt.</li><li><b>Antwortoptionen sowie Festlegung der Bepunktung.</b><br>Die Antworten stehen einfach im Text hintereinander. '~' trennt zwei Antwortoptionen; '=' trennt die Punkte von der Option. Die Reihenfolge der Antwortoptionen ist relevant! Optional bei SHORTANSWER, dann jedoch keine automatische Bewertung.</li></ol><b>Hinweise:</b><ul><li>Die Breite der Eingabefelder wird automatisch berechnet: Breite mindestens 20 Zeichen, sonst maximale Länge der Antwortoptionen sowie der zuletzt eingegebenen Lösung (+3). Sollen die Felder gleich lang sein, kann ggf. überall eine unbenötige falsche lange Lösung hinterlegt werden (z.B. &quot;~0=012345678901234567890123465789&quot;).</li></ul><b>Beispiele:</b><br><ul><li>Die ehemalige deutsche Bundeshauptstadt {SHORTANSWER:1=Bonn} trägt heute den Titel Bundesstadt.<br>Nur die Eingabe von Bonn erhält 1 Punkt.</li><li>San Francisco: {MULTICHOICE:1=Kalifornien~0=Arizona}</li><li>4+4.5 = {NUMERICAL:1=8.5:.001~.5=8:1}<br>Werte im Bereich 8,5±0,001 erhalten 1 Punkt; 8±1 ergeben 0,5 Punkte.</li></ul></span></td>");
			out.println("</tr>");
		}
		out.println("<tr>");
		...
		out.println("<td><input type=text size=5 id=maxSubmitters name=\"maxSubmitters\" value=\"" + task.getMaxSubmitters() + "\" onkeyup=\"if (document.getElementById('maxSubmitters').value>1) {document.getElementById('submitteracrossgroups').style.display=null;} else {document.getElementById('submitteracrossgroups').style.display='none';}return true;\"><span id=submitteracrossgroups" + (task.getMaxSubmitters() > 1 ? "" : " style=\"display:none;\"") + ">, <input type=checkbox id=submitteracrossgroupscb name=allowSubmittersAcrossGroups " + (task.isAllowSubmittersAcrossGroups() ? "checked" : "") + "> <label for=submitteracrossgroupscb>über Gruppengrenzen hinweg</label></span> <a href=\"#\" onclick=\"toggleVisibility('maxsubmittershelp'); return false;\">(?)</a><br><span style=\"display:none;\" id=maxsubmittershelp><b>Hilfe:</b><br>Sofern &quot;über Gruppengrenzen hinweg&quot; nicht gesetzt ist, müssen die Studierenden in Gruppen eingeteilt sein und können auch nur Studierende wählen, die in der gleichen Gruppe sind. Die Zahl wird als Gesamtanzahl der Studierenden, die eine Aufgabe gemeinsam bearbeiten dürfen, angesehen. Sind bei der Veranstaltung Abgabegruppen defininert und wird eine Zahl &gt; 1 angegeben, werden immer alle Studierenden der Gruppe bei der ersten Abgabe automatisch hinzugefügt (auch wenn mehr Studierende in der Gruppe sind; ist der abgebende Studierende in keiner Gruppe ist es automatisch eine Einzelabgabe, wenn gruppenübergreifende PartnerInnen verboten sind); die Angabe von &quot;1&quot; erlaubt auch bei Abgabegruppen Individualabgaben. Im Fall von Abgabegruppen können Studierende, die in einer Abgabegruppe sind, keine gruppenübergreifenden oder beliebigen Partnerabgaben durchführen (auch nicht, wenn gruppenübergreifende Partnerschaften erlaubt sind).</span></td>");
		out.println("</tr>");
		if (task.getTaskid() != 0) {
			out.println("<tr>");
		...
		out.println("<td><input type=checkbox id=pointsmanual name=pointsmanual " + (task.getShowPoints() == null ? "checked" : "") + "> <label for=pointsmanual>manuell freischalten</label> oder <input type=text name=pointsdate pattern=\"([012][1-9]|[123][01])\\.[01][0-9]\\.[0-9]{4}( ([0-1][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9])?\" value=\"" + pointsDate + "\"> (dd.MM.yyyy oder dd.MM.yyyy HH:mm:ss)</td>");
		out.println("</tr>");
		if (task.getTaskid() != 0 && !task.isSCMCTask()) {
			out.println("<tr>");
		...
		out.println("<td><input type=checkbox id=prematureClosing name=prematureClosing" + (task.isAllowPrematureSubmissionClosing() ? " checked" : "") + "> <label for=prematureClosing>Aufgabe kann vor der Deadline durch die Studierenden als abgeschlossen markiert werden</label></td>");
		out.println("</tr>");
		if (task.getTaskid() == 0) {
			out.println("<tr>");
